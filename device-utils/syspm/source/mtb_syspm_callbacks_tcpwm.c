    ...
    cy_en_syspm_callback_mode_t mode)
{
    ...
    switch (mode)
    {
        case CY_SYSPM_BEFORE_TRANSITION:
        {
            if (_mtb_syspm_tcpwm_is_enabled(params, context->channelNum))
            {
                ...
                if (CY_TCPWM_PWM_STATUS_COUNTER_RUNNING &
                    Cy_TCPWM_PWM_GetStatus((TCPWM_Type const*)params->base, (context->channelNum)))
                {
        ...
        case CY_SYSPM_AFTER_TRANSITION:
        {
            ...
            {
                Cy_TCPWM_Enable_Single((TCPWM_Type*)params->base, (context->channelNum));

                // Trigger the TCPWM if it was running before sleep
                if (context->preSleepRunning)
                {
                    Cy_TCPWM_TriggerStart_Single((TCPWM_Type*)params->base, (context->channelNum));
                }
            }
