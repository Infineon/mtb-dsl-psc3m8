    ...
    cy_en_syspm_callback_mode_t mode)
{
    ...
    switch (mode)
    {
        case CY_SYSPM_CHECK_READY:
            ...
            if ((0UL ==
                 (CY_SCB_UART_TRANSMIT_ACTIVE &
                  Cy_SCB_UART_GetTransmitStatus((CySCB_Type const*)params->base,
                                                context->uart_context))) &&
                (0UL ==
                 (CY_SCB_UART_RECEIVE_ACTIVE  &
                  Cy_SCB_UART_GetReceiveStatus((CySCB_Type const*)params->base,
                                               context->uart_context)))
...

            {
                 ...
                 * has contents.
                 */
                uint32_t txMasked = Cy_SCB_GetTxInterruptStatusMasked(
                    (CySCB_Type const*)params->base);
                uint32_t rxMasked = Cy_SCB_GetRxInterruptStatusMasked(
                    (CySCB_Type const*)params->base);
                ...
                if (Cy_SCB_UART_IsTxComplete((CySCB_Type const*)params->base)
                    && (0UL == Cy_SCB_UART_GetNumInRxFifo((CySCB_Type const*)params->base))
        ...
        case CY_SYSPM_BEFORE_TRANSITION:
            allow = true;
            Cy_SCB_UART_Disable((CySCB_Type*)params->base, context->uart_context);
            break;
        ...
        case CY_SYSPM_AFTER_TRANSITION:
            allow = true;
            restore_pins(context);
            Cy_SCB_UART_Enable((CySCB_Type*)params->base);
            break;
