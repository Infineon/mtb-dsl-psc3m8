...
cy_en_syspm_boot_mode_t Cy_SysPm_GetBootMode(void)
{
    ...
    /* Warm/Cold boot can only be detected when no reset cause at all is latched.
     * RES_CAUSE and RES_CAUSE2 record disjoint reset sources, so a genuine reset
     * leaves one register non-zero and the other zero - both must be zero here. */
