...
# For COMBINED projects use APPNAME so the IDE post-build and the command-line
# build produce the same project key in memreport.json (avoids duplicate entries).
# For non-COMBINED (multi-project) builds keep the directory name so IDE exports
# accumulate data even when CY_IDE_PRJNAME forces identical ELF basenames.
ifeq (COMBINED,$(MTB_TYPE))
_MTB_RECIPE__MEMREPORT_PROJECT_ELF_PAIRS:=$(APPNAME)=$(call mtb__path_normalize,$(MTB_IDE__TARG_FILE))
else
