...
# End address is exclusive for objcopy --pad-to and for srec_cat -crop/-fill.
ifneq (,$(filter $(VCORE_ATTRS),CRC_PPCA))
_MTB_RECIPE__CRC_CALC_END=$(shell printf "0x%X\n" $$(( ($(MTB_BSP__CRC_CALC_START_ADDR) + $(MTB_BSP__CRC_CALC_SIZE)) & 0xFFFFFFFF )) )
endif
...
ifeq ($(TOOLCHAIN),LLVM_ARM)
	...
	$(MTB_TOOLCHAIN_GCC_ARM__OBJCOPY) -O binary "$<" "$(MTB_TOOLS__OUTPUT_CONFIG_DIR)/$(APPNAME).bin" --pad-to $(_MTB_RECIPE__CRC_CALC_END) --gap-fill 0x00
	$(MTB__NOISE)$(CY_TOOL_srec_cat_EXE_ABS) "$(MTB_TOOLS__OUTPUT_CONFIG_DIR)/$(APPNAME).bin" -binary -crop $(MTB_BSP__CRC_CALC_START_ADDR) $(_MTB_RECIPE__CRC_CALC_END) -fill 0x00 $(MTB_BSP__CRC_CALC_START_ADDR) $(_MTB_RECIPE__CRC_CALC_END) -CRC32_Little_Endian $(MTB_BSP__CRC_START_ADDR) -o "$(MTB_TOOLS__OUTPUT_CONFIG_DIR)/$(APPNAME).bin" -binary
