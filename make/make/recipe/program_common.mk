...
# Device identifier passed to mtb-programmer via --mpn. For KitProg3 this is the
# device MPN. For J-Link, recipes override this with the J-Link device alias
# (mtb-programmer accepts an alias in place of an MPN). Recipes that do not
# define a J-Link alias fall back to $(DEVICE).
_MTB_RECIPE__ADVANCED_PROGRAM_MPN?=$(DEVICE)

# Programming interfaces accepted by the advanced_program target.
# - Recipes that do not support mtb-programmer at all set
#   _MTB_RECIPE__ADVANCED_PROGRAM_UNSUPPORTED before including this file.
# - Recipes that have validated the mtb-programmer J-Link flow (device alias via
#   _MTB_RECIPE__ADVANCED_PROGRAM_MPN and J-Link-compatible
#   _MTB_RECIPE__ADVANCED_PROGRAM_EXTRA_ARGS) opt in by setting
#   _MTB_RECIPE__ADVANCED_PROGRAM_SUPPORTS_JLINK before including this file.
ifneq (,$(_MTB_RECIPE__ADVANCED_PROGRAM_UNSUPPORTED))
_MTB_RECIPE__ADVANCED_PROGRAM_INTERFACES:=
else
_MTB_RECIPE__ADVANCED_PROGRAM_INTERFACES:=KitProg3 $(if $(_MTB_RECIPE__ADVANCED_PROGRAM_SUPPORTS_JLINK),JLink)
endif

# Tells mtb-programmer to look for a J-Link probe. KitProg3 devices are excluded
# from the search when this option is given.
_MTB_RECIPE__ADVANCED_PROGRAM_JLINK_OPT=$(if $(filter JLink,$(_MTB_RECIPE__PROGRAM_INTERFACE_SUBDIR)),--jlink)

advanced_program:
ifeq (,$(_MTB_RECIPE__ADVANCED_PROGRAM_INTERFACES))
	$(error The advanced_program target is not supported for this recipe. \
	Use the "program" target instead)
else ifeq (,$(filter $(_MTB_RECIPE__ADVANCED_PROGRAM_INTERFACES),$(_MTB_RECIPE__PROGRAM_INTERFACE_SUBDIR)))
	$(error The advanced_program target requires one of these interfaces: \
	$(_MTB_RECIPE__ADVANCED_PROGRAM_INTERFACES). \
	Current interface is "$(_MTB_RECIPE__PROGRAM_INTERFACE_SUBDIR)". \
	Set BSP_PROGRAM_INTERFACE to a supported interface, \
	or use the "program" target instead)
endif
ifeq ($(LIBNAME),)
	$(MTB__NOISE)echo;\
	echo "Programming target device with mtb-programmer...";\
	"$(CY_TOOL_mtb-programmer_EXE_ABS)" \
		$(_MTB_RECIPE__ADVANCED_PROGRAM_JLINK_OPT) \
		--serial "$(MTB_PROBE_SERIAL)" \
		--hexfile "$(call mtb__path_normalize,$(_MTB_RECIPE__ADVANCED_PROGRAM_HEX_FILE))" \
		--mpn $(_MTB_RECIPE__ADVANCED_PROGRAM_MPN) \
